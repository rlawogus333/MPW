...
처음에는 칩을 5 V로 구동하고 TXS0108E 레벨 시프터로 3.3 V ↔ 5 V를 변환할 계획이었고([`legacy`](../test/legacy/sram_test_5v_txs0108e.py)),
이후 칩 VDD를 3.3 V로 낮춰 GPIO를 직결하는 안으로 바꿨습니다.

**실제 측정(2026-09)은 두 안의 중간**입니다 — 테스트 보드가 칩 `VDD`를 **USB 5 V**로 공급하고,
라즈베리파이 **3.3 V GPIO를 레벨 시프터 없이 그대로** 칩 입력에 연결했습니다. 이 구성에서 PRE · WE · SE가
논리 1로 정상 인식되어 write/read가 모두 동작했습니다 ([06_silicon_measurement.md](06_silicon_measurement.md)).

| | 실측 구성 (VDD 5 V + 3.3 V 직결) | VDD 3.3 V 직결 | 5 V + TXS0108E (초기안) |
|---|---|---|---|
| 부품 | 없음 | 없음 | TXS0108E × 2, 바이패스 커패시터, 풀다운 저항 |
| 입력 high 마진 | 작음 (3.3 V < 0.7 VDD) | 충분 | 충분 |
| 입력단 정적 전류 | 있을 수 있음 | 없음 | 없음 |
| RDATA → Pi 입력 | **불가** (RDATA ≈ 4.3 V > 3.3 V 허용치) | 가능 | 가능 |
| 실리콘 검증 | ✅ 완료 | 미확인 | — |

마지막 행이 중요합니다. VDD 5 V에서는 `RDATA`가 약 4.3 V까지 올라가므로 **라즈베리파이 GPIO 입력에 직접 연결하면 안 됩니다**.
2026-09 측정에서는 RDATA를 오실로스코프로만 관찰했습니다. `test/sram_rw_test.py`처럼 Pi가 RDATA를 읽으려면
VDD를 3.3 V로 낮추거나, RDATA 쪽에 분압 저항·레벨 시프터를 넣어야 합니다.

## 2. GPIO 핀맵 (BCM 번호)

`test/sram_rw_test.py`(전체 스위트)의 `PINS` 딕셔너리와 동일합니다.
2026-09 실측에서는 이 중 **PRE(17) · WE(8) · SE(7)만** 사용했고, E · A · WDATA는 보드에서 고정, RDATA는 스코프로 관찰했습니다.
또한 제작된 칩에는 `A[3]` 핀이 없습니다 ([01 §2.1](01_architecture.md#21-제작된-칩-a20-a0-공유)).
...
| `A[1]` | 23 | OUT | Row addr |
| `A[2]` | 24 | OUT | Row addr MSB (칩 기준) |
| `A[3]` | 25 | OUT | 칩에 해당 핀 없음 (원안 기준) |
| `WDATA[0]` | 12 | OUT | |
| `WDATA[1]` | 16 | OUT | |
| `WDATA[2]` | 20 | OUT | |
| `WDATA[3]` | 21 | OUT | |
...
| 칩 핀 | 연결 |
|-------|------|
| `VDD` | 실측: 테스트 보드 USB 5 V (점퍼, 1 nF) · 3.3 V 구동 시 라즈베리파이 3.3 V 레일 또는 LDO |
| `E` | 실측: USB 5 V 점퍼로 HIGH 고정 |
